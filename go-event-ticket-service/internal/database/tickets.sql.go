...
const getTicketsByIds = `-- name: GetTicketsByIds :many
select
	t.id,
	t.title,
	t.price,
...
from
	tickets t
WHERE t.id IN (/*SLICE:ticket_ids*/?)
`

type GetTicketsByIdsRow struct {
	ID            string
	Title         string
	Price         float64
	Status        TicketsStatus
...
func (q *Queries) GetTicketsByIds(ctx context.Context, ticketIds []string) ([]GetTicketsByIdsRow, error) {
	query := getTicketsByIds
	...
	defer rows.Close()
	var items []GetTicketsByIdsRow
	for rows.Next() {
		var i GetTicketsByIdsRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Price,
